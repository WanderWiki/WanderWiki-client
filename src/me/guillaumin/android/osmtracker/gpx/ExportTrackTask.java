	...
	*/
	...
	private void exportTrackAsGpx(long trackId) throws ExportTrackException {
		...
		if (sdRoot.canWrite()) {
		    ...
		    String[] allColumns = {Schema.COL_ID,Schema.COL_PSEUDONYM,Schema.COL_DATABASE_ID};
		    final Cursor cAuthor = dbHelper.getWritableDatabase().query(Schema.TBL_USER,
			...
			if (null != cTrackPoints && null != cWayPoints) {
				...
				try {
					writeGpxFile(cTrackPoints, cWayPoints, cAuthor, cTrack,trackFile);
					if (exportMediaFiles()) {
	 ...
	 * @throws IOException 
	 */
	private void writeGpxFile(Cursor cTrackPoints, Cursor cWayPoints, Cursor cAuthor,Cursor cTrack, File target) throws IOException {
		...
		try {
			...
			/////
			writer.write("\t" + "<title>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_NAME))+"</title>");
			writer.write("\t" + "<tags>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_TAGS))+"</tags>");
			writer.write("\t" + "<length>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_LENGTH))+"</length>");
			writer.write("\t" + "<description>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_DESCRIPTION))+"</description>");
			writer.write("\t" + "<articles>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_ARTICLES))+"</articles>");
			writer.write("\t" + "<startLat>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_START_LAT))+"</startLat>");
			writer.write("\t" + "<startLng>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_START_LON))+"</startLng>");
			writer.write("\t" + "<endLat>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_END_LAT))+"</endLat>");
			writer.write("\t" + "<endLng>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_END_LON))+"</endLng>");
			writer.write("\t" + "<time>" +  cTrack.getString(cTrack.getColumnIndex(Schema.COL_START_DATE))+"</time>");
			
			writer.write("\t" + "<idAuthor>" +  cAuthor.getString(cAuthor.getColumnIndex(Schema.COL_DATABASE_ID))+"</idAuthor>");
