...
public class DataHelper {
	 ...
	 *            Language of user
	 */
	public void user(String pseudonym, String lang,String gmail) {
		//Log.v(TAG, "Tracking (trackId=" + trackId + ") location: " + location);
		ContentValues values = new ContentValues();
	...
	//	values.put(Schema.COL_TRACK_ID, trackId);
		...
		values.put(Schema.COL_LANG, lang);

		

	    values.put(Schema.COL_TIMESTAMP, System.currentTimeMillis());
	    
		Uri trackUri = ContentUris.withAppendedId(TrackContentProvider.CONTENT_URI_USER, 1);
		contentResolver.insert(Uri.withAppendedPath(trackUri, Schema.TBL_USER), values);
	}
	...
	/**
	 * Change the author of this track.
	 * @param trackId Id of the track
	 * @param author  New name of author, or null to clear it
	 * @param cr  Database connection for query
	 */
	public static void setTrackAuthor(long trackId, String author, ContentResolver cr) {
		Uri trackUri = ContentUris.withAppendedId(TrackContentProvider.CONTENT_URI_TRACK, trackId);
		ContentValues values = new ContentValues();
		values.put(Schema.COL_AUTHOR, author);
		cr.update(trackUri, values, null, null);		
	}
