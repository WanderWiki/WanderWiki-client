...
public class GPSLogger extends Service implements LocationListener {
	 ...
	 * 
	 */
	public float distance_changing;
	/**
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (OSMTracker.INTENT_TRACK_WP.equals(intent.getAction())) {
				...
				if (extras != null) {
					...
					if (lastLocation ==null) distance_changing=0;
	...
	public void onCreate() {	
		...
		lmgr.requestLocationUpdates(LocationManager.GPS_PROVIDER, 7000, 10, this);
	 ...
	 * Start GPS tracking.
	 */
	public void startTracking(long trackId) {
	...
	//	    distance_changing += lastLocation.distanceTo(location);
	//	    if (distance_changing > 300 && distance_changing >lastLocation.getAccuracy())
	//	    	{
	//	    	isMoving =1 ;
	//	    	distance_changing=0;
	//	    	}
	//	    else isMoving = 0;&& lastLocation.distanceTo(lastLocation) <lastLocation.getAccuracy()
			...
			if (isTracking == true ) {
				...
				if (lastLocation ==null) distance_changing=0;
				else distance_changing =  location.distanceTo(lastLocation);
			    ...
			    ContentValues values = new ContentValues();
			 	cursor.moveToFirst();
			 	Long distance_changing_long =(long) distance_changing;
			 	Long new_distance = distance_changing_long + cursor.getLong(cursor.getColumnIndex(Schema.COL_LENGTH));
			    values.put(Schema.COL_LENGTH, new_distance);
		...
		//	lmgr.requestSingleUpdate(LocationManager.GPS_PROVIDER, 7000, 4*max(Double.valueOf(accuracy),Double.valueOf(lastLocation.getAccuracy())).floatValue(), this);
		}
	}
	...
	private Notification getNotification() {
		Notification n = new Notification(R.drawable.logo, getResources().getString(R.string.notification_ticker_text), System.currentTimeMillis());
			
		Intent startDisplayTrackMap= new Intent(this, DisplayTrackMap.class);
		startDisplayTrackMap.putExtra(TrackContentProvider.Schema.COL_TRACK_ID, currentTrackId);
		PendingIntent contentIntent = PendingIntent.getActivity(this, 0, startDisplayTrackMap, PendingIntent.FLAG_UPDATE_CURRENT);
